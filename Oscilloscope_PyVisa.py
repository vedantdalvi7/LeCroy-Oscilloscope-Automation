...
class Oscilloscope:
    ...
    def set_channel_Bandwidth_Limit(self, channel:int, value:str):
        ...
        if value.lower() not in {t.lower() for t in Limit_NAMES_VALID}:
             raise ValueError(f'Vertical unit must be one of {Limit_NAMES_VALID},  received {repr(value)}...')
        self.write(f"vbs? 'app.Acquisition.C{channel}.BandwidthLimit = {value})'") 

    def set_Channel_Labels(self, channel:int, label:str, view_label:bool):
        """Set the channel label"""
        _validate_channel_number(channel)
        if not isinstance(label, str):
            raise TypeError(f'Vertical unit must be a string, received object of type {type(label)}.')
    ...
    def clear_sweeps(self):
        self.write("vbs? 'app.Preferences.Display.ClearSweeps'")
    ...
    def timebase_settings(self, sampling_mode:str,t_div:str, real_time_memory:str, sample_points:float, active_channels:str):
        ...
        if not isinstance(sampling_mode, str):
            raise TypeError(f'The format type must be a string, received ibject of type {type(sampling_mode)}')
        ...
        #     raise TypeError(f'The format type must be a string, received ibject of type {type(sampling_rate)}')
        # self.write(f"vbs 'app.Acquisition.Horizontal.SamplingRate = {sampling_rate}'")
        #Set the Times/div
        if not isinstance(t_div, str):
            raise TypeError(f'The format type must be a string, received ibject of type {type(t_div)}')
        ...
        if not isinstance(real_time_memory, str):
            raise TypeError(f'The format type must be a string, received ibject of type {type(real_time_memory)}')
        ...
        if not isinstance(sample_points, float):
            raise TypeError(f'The format type must be a float number, received ibject of type {type(sample_points)}')
        ...
        if not isinstance(active_channels, str):
            raise TypeError(f'The format type must be a string, received ibject of type {type(active_channels)}')
    ...
    def save_parameters_trace(self, path:str, channel: int, fileformat:str, id:str):
        '''MEASURES AND SAVES THE StdVer PARAMETERS OF THE TRIGGERED/SPECIFIED CHANNEL AND SAVES THEM IN A EXCEL FILE 
    ...
    def save_waveform_on_PC(self, path:str, waveform:pd.DataFrame, id:str, max_sample_points:float): #CHECK
